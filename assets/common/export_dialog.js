...
.export-actions { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) scale(0.8); background: var(--weui-BG-1); border-radius: 12px; padding: 20px; min-width: 240px; max-height: 80vh; display: flex; flex-direction: column; opacity: 0; transition: opacity 0.2s, transform 0.2s; pointer-events: auto; }
.export-actions.active { opacity: 1; transform: translate(-50%, -50%) scale(1); }
.export-title { position: relative; font-size: 16px; color: var(--weui-FG-0); margin-bottom: 16px; font-weight: 500; flex-shrink: 0; }
...
export function showExportDialog({ title, options, onChange = () => {} }) {
    ...
    wrap.innerHTML = `
        ...
        <div class="export-actions">
            ...
            <div class="export-body">${optionsHtml}</div>
        </div>
    `;
